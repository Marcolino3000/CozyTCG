...
| `CozyTGC.EditorTools.CardWearPreviewRenderer.RenderPreview` | Render Wear Preview |
| `CozyTGC.EditorTools.PixelArtCardImporter.ApplyToAllCardTextures` | Apply Pixel Art Import Settings |

Both preview renderers render through a camera, so drop `-nographics` for them. They write `CardFoilPreview.png` / `CardWearPreview.png` next to `Assets/` (`COZY_PREVIEW_PATH` / `COZY_WEAR_PREVIEW_PATH` override the paths) — the fastest way to eyeball a shader change without a human pressing Play. The foil sheet is five tiers at four angles; the wear sheet is **one card filling each frame**, because damage authored at 73x113 vanishes at the size a five-card row leaves. `Render Wear Preview` also logs the grading ramp, which is where `CardCondition`'s full-scale constants have to be re-measured from whenever ageing changes.
...
**Wear is one map, not five effects.** Scuffs, ink loss, dents, creases and chips are four channels of a single RGBA texture per face at the card's own 73x113, written only by `CardWear`; bending is three scalars and lives in the vertex shader, because it has to change the silhouette. R and G are per face so a card must be restored on both sides, but **B (height) and A (missing) always come from the front map** — a dent goes through the paper, and `DepthOnly` only ever sees the front, so a chip it could not see would write depth over a pixel the forward pass already clipped. The height channel is what earns its keep: every foil layer is driven off `tilt`, so perturbing the normal breaks the rainbow, sweep, sparkle and chrome over a dent at once without any of them knowing wear exists. Nothing is smoothed anywhere — wear is sampled through the same pixel-snapped UV as the artwork and the dent normals are one-per-card-pixel blocky on purpose. `CardQuad.asset` is a 16x24 grid rather than the four vertex quad it was, because a vertex shader can only move vertices that exist, and `#pragma target` is 3.5 rather than 3.0 because the vertex stage samples a texture now. `CardApplyBend` (forward) and `CardApplyBendPosition` (DepthOnly) must agree exactly, the same discipline the keyword pragmas need.

**Restoring is the same code read backwards.** `CardWear.Rub` is the only edit path; `Age` stamps damage in from a seed, a tool rubs it out. A `RestorationTool` is nothing but rates, and every tool damages something else while it works — the pad lifts print, the burnisher polishes, and every tool charges `overworkScuff` on any texel where it has nothing left to do. That is what orders the steps (press, burnish, abrade, fill, re-ink, polish) without a single check enforcing it, so changing a rate changes the puzzle. The map is CPU-side `float` texels uploaded whole, and saving stores the seed plus the stroke list rather than the map, because both halves are deterministic.
